...
export default function Page() {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            ...
            console.log("Potential Email Error: ", potentialEmailError);
            if (isSignUpError(potentialEmailError)) {
                let error = determineErrorMessage(potentialEmailError);
                setEmailError(error);
                return;
            }
            // If there is no error, set the email error to an empty string
            setEmailError("");
        }, 500);
    ...
    useEffect(() => {
        const timeout = setTimeout(() => {
            ...
            // If there is no error, set the password error to an empty string
            setPasswordError("");
        }, 100);
        return () => clearTimeout(timeout);
    }, [password]);
